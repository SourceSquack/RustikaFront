...
export default function CarouselDiscounts({ results, title }) {
    const scrollLeft = () => {
    ...
    return (
        <div className='relative max-w-6xl mx-auto'>
            <div className='text-center text-olive py-6 text-4xl font-bold'>{title}</div>
            ...
            <div id={title} className='carpusel p-4 flex items-center justify-start overflow-x-auto scroll-smooth'>
                {results.map((result, index) => (
                    <div key={index}>
                        <CardDiscounts key={result._id} result={result} />
                    </div>
                ))}
            </div>
        </div>
